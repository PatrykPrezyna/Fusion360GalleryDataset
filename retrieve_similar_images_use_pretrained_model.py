"""
Example script for retrieving similar images using a pretrained vision model (CLIP).

Usage:
    python retrieve_similar_images_use_pretrained_model.py --query "path/to/image.png" --image-dir "path/to/images" --top-k 5
"""
...
def load_pretrained_model(device="cpu"):
    ...
    Note: Using CLIP (Contrastive Language-Image Pretraining) model which is
    specifically designed for image similarity tasks. The AutoGluon fev-bench
    space is for time series forecasting, not image retrieval, so CLIP is used
    instead as it's the standard pretrained model for image similarity search.
    """
    print("Loading pretrained CLIP model from Hugging Face...")
...
def retrieve_similar_images_pretrained(
    ...
    """
    Retrieve similar images using pretrained CLIP model.
...
def main():
    parser = argparse.ArgumentParser(
        description="Retrieve similar images using pretrained CLIP model"
    )
    parser.add_argument(
        "--query",
        type=str,
    ...
    if os.path.isfile(args.query):
        ...
        query_images = [q for q in query_images if not q.endswith('_retrieval_results_clip.png')]
    ...
    for idx, query_image in enumerate(query_images, 1):
        ...
        save_path = os.path.join(results_folder, f"{query_name}_retrieval_results_clip.png")
    ...
    if len(query_images) > 1:
        combine_retrieval_results(query_folder, model_name="clip")


def combine_retrieval_results(query_folder: str, model_name: str = "clip"):
    """
