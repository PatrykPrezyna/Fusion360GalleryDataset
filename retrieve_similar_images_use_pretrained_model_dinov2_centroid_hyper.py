...
def load_pretrained_model(device="cpu", model_variant="base", image_size=224):
    """
    ...
    Args:
        ...
        model_variant: Model size variant. Options:
            - "small": dinov2-small (21M params, 384 dim, fastest)
            - "base": dinov2-base (86M params, 768 dim, default, good balance)
            - "large": dinov2-large (300M params, 1024 dim, better accuracy)
            - "giant": dinov2-giant (1.1B params, 1536 dim, best accuracy, slowest)
    ...
    if model_variant not in model_map:
        raise ValueError(f"Invalid model_variant '{model_variant}'. Must be one of: {list(model_map.keys())}")
    
    model_name = model_map[model_variant]
    print(f"Loading pretrained DINOv2 model '{model_name}' from Hugging Face...")
    ...
    model.eval()
    print(f"Model loaded successfully! Variant: {model_variant}, Embedding dim: {model.config.hidden_size}, Image size: {image_size}x{image_size}")
...
def extract_image_embeddings_with_centroids(model, processor, image_paths, device="cpu", batch_size=32, pooling_strategy="cls", image_size=224):
    """
    ...
    Args:
        ...
        device: Device to run inference on
        batch_size: Batch size for processing
        pooling_strategy: Pooling strategy for patch tokens
        image_size: Image size for fallback images
    ...
    for i in range(0, len(image_paths), batch_size):
        ...
        with torch.no_grad():
            outputs = model(**inputs)
            hidden_states = outputs.last_hidden_state  # Shape: (batch_size, num_patches + 1, hidden_dim)
            
            # Apply pooling strategy
            if pooling_strategy == "cls":
                # CLS token only (first token, index 0)
                image_features = hidden_states[:, 0, :]
            elif pooling_strategy == "mean":
                # Average of all patch tokens (exclude CLS token)
                image_features = hidden_states[:, 1:, :].mean(dim=1)
            elif pooling_strategy == "cls+mean":
                # Concatenate CLS and mean of patches
                cls_token = hidden_states[:, 0, :]
                mean_patches = hidden_states[:, 1:, :].mean(dim=1)
                image_features = torch.cat([cls_token, mean_patches], dim=1)
            elif pooling_strategy == "weighted_mean":
                # Weighted average with higher weight for center patches
                num_patches = hidden_states.shape[1] - 1  # Exclude CLS
                patch_size = int(np.sqrt(num_patches))  # Assume square patch grid
                weights = torch.ones(hidden_states.shape[1] - 1, device=hidden_states.device)
                ...
                for i in range(patch_size):
                    for j in range(patch_size):
                        ...
                        # Gaussian-like weighting (higher weight for center)
                        weights[idx] = np.exp(-dist**2 / (2 * (patch_size / 4)**2))
                weights = weights / weights.sum()  # Normalize
                # Weighted average of patch tokens
                image_features = (hidden_states[:, 1:, :] * weights.unsqueeze(0).unsqueeze(-1)).sum(dim=1)
            elif pooling_strategy == "max":
                # Max pooling across patches (exclude CLS token)
                image_features, _ = hidden_states[:, 1:, :].max(dim=1)
            else:
...
def extract_query_embeddings(model, processor, query_image_paths: list, device: str = "cpu", pooling_strategy: str = "cls"):
    """
...
def retrieve_similar_images_with_centroids(
    ...
    centroid_dim = centroid_embeddings.shape[1]
    query_dim = query_embedding.shape[1]
    if centroid_dim != query_dim:
        raise ValueError(
            f"Dimension mismatch: centroid embeddings have dimension {centroid_dim}, "
            f"but query embedding has dimension {query_dim}. "
            f"This usually happens when:\n"
            f"  1. Cached embeddings were created with a different model variant or pooling strategy\n"
            f"  2. Query is using a different model variant or pooling strategy than the cached embeddings\n"
            f"  Solution: Delete cached embeddings and recompute with the same model/pooling settings, "
            f"or use the same model variant and pooling strategy for both."
        )
...
def main():
    ...
    parser.add_argument(
        ...
        help="DINOv2 model variant: small (fastest, 384 dim), base (default, 768 dim), "
             "large (1024 dim), giant (best accuracy, 1536 dim, slowest)"
    )
    parser.add_argument(
        "--pooling-strategy",
        type=str,
        default="cls",
        choices=["cls", "mean", "cls+mean", "weighted_mean", "max"],
        help="Pooling strategy for patch tokens: "
             "cls (CLS token only, default), "
             "mean (average patches), "
             "cls+mean (concatenate CLS and mean), "
             "weighted_mean (center-weighted average), "
             "max (max pooling)"
    )
    parser.add_argument(
        "--temperature",
        type=float,
        default=1.0,
        help="Temperature scaling for similarity scores (default: 1.0, no scaling). "
             "> 1.0 makes scores softer, < 1.0 makes them sharper"
    )
    parser.add_argument(
        "--batch-size",
        type=int,
        default=32,
        help="Batch size for processing images (default: 32). Increase for faster processing if you have GPU memory"
    )
    parser.add_argument(
        "--image-size",
        type=int,
    ...
    model, processor = load_pretrained_model(device=device, model_variant=args.model_variant, image_size=args.image_size)
    ...
    if cache_centroids_file and os.path.exists(cache_centroids_file) and cache_mapping_file and os.path.exists(cache_mapping_file):
        ...
        # Check if cached embeddings match current model/pooling configuration
        cached_embedding_dim = centroid_embeddings.shape[1]
        # Calculate expected embedding dimension based on model variant and pooling strategy
        model_map = {
            "small": 384,
            "base": 768,
            "large": 1024,
        ...
        base_dim = model_map.get(args.model_variant, 768)
        if args.pooling_strategy == "cls+mean":
            expected_dim = base_dim * 2
        else:
            expected_dim = base_dim
        
        if cached_embedding_dim != expected_dim:
            print(f"Warning: Cached embeddings dimension ({cached_embedding_dim}) doesn't match "
                  f"current configuration (model={args.model_variant}, pooling={args.pooling_strategy}, "
                  f"expected_dim={expected_dim}).")
            print("Recomputing embeddings with current configuration...")
            embeddings_loaded_from_cache = False
            if benchmark_timer:
                benchmark_timer.start_embedding_extraction()
            centroid_embeddings, centroid_to_paths, base_id_to_centroid_idx = extract_image_embeddings_with_centroids(
                model, processor, image_paths, device=device, batch_size=args.batch_size, pooling_strategy=args.pooling_strategy, image_size=args.image_size
            )
            ...
            if args.cache_paths and os.path.exists(args.cache_paths):
                ...
                if len(cached_paths) == len(image_paths) and cached_paths == image_paths:
                    print("Cached centroid embeddings match current image set and configuration.")
                else:
                    ...
                    if benchmark_timer:
                        benchmark_timer.start_embedding_extraction()
                    centroid_embeddings, centroid_to_paths, base_id_to_centroid_idx = extract_image_embeddings_with_centroids(
                        model, processor, image_paths, device=device, batch_size=args.batch_size, pooling_strategy=args.pooling_strategy
                    )
                if cache_centroids_file:
                    torch.save(centroid_embeddings, cache_centroids_file)
                if cache_mapping_file:
    ...
    for idx, result in enumerate(results):
        ...
        try:
            ...
            ax.set_title(f"{similarity:.3f}{rank_text}{match_text}",
                         fontsize=40, pad=5, color=title_color,
                         fontweight='bold' if is_match else 'normal')
            ax.axis('off')
        except Exception as e:
